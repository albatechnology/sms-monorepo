            ...
            {{-- <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('channel_category') ? 'is-invalid' : '' }}" name="channel_category_id" id="channel_category_id" required>
                    @foreach($channel_categories as $id => $channel_category)
                        <option value="{{ $id }}" {{ (old('channel_category_id') ? old('channel_category_id') : $channel->channel_category->id ?? '') == $id ? 'selected' : '' }}>{{ $channel_category }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('company') ? 'is-invalid' : '' }}" name="company_id" id="company_id" required>
                    @foreach($companies as $id => $company)
                        <option value="{{ $id }}" {{ (old('company_id') ? old('company_id') : $channel->company->id ?? '') == $id ? 'selected' : '' }}>{{ $company }}</option>
                    @endforeach
                </select>
